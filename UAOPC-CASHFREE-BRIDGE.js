...
winston.configure({
  transports: [
    new winston.transports.File({ name: 'info', filename: 'coinmonster2cashfree-info.log', level: 'info' }),
    new winston.transports.File({ name: 'error', filename: 'coinmonster2cashfree-error.log', level: 'error' }),
...
class RunningTransaction {
  ...
  async start() {
    ...
    try {
      ...
      winston.info("RunningTransaction[%d]::start()", this.instance);

      // set 'startBit' to false
      await this.opc.writeAsync(global_config.sharedio.startbit, false);

      // select the config by index [instance]
      this.instance_config = global_config.ios[this.instance];
      winston.debug("RunningTransaction[%d]::start config: ", this.instance,this.instance_config);

      // --- set (errorCode, errorDescription, paymentURL, transactionID, transactionSigned, cancelBit) to empty or false
      ...
      // --- get the (apiKey, profileID, apiLocation)
      ...
      // --- get the (transactionAmount, transactionDescription)
      ...
      winston.info("RunningTransaction[%d]::api_internetpayments_post:", this.instance, paymentdata);

      // --- request transaction 'api/internetpayments/':POST from Cashfree 
      this.response = await this.api_internetpayments_post(paymentdata);
      if (this.response.error) {
        ...
        winston.error("RunningTransaction[%d]::api_internetpayments_post:", this.instance, code, description, detail);
      ...
      // append apiSuffix (TODO: recheck this from production)
      this.response.paymentURL += global_config.cashfree.apiSuffix;

      // set the transactionID, paymentURL to the PLC which where received from Cashfree
      ...
      winston.info("RunningTransaction[%d]::api response transactionID=%s", this.instance, this.response.transactionId);

      // if we use a dynamic QRCODE open the browser (not used anymore)
      ...
      do {
        ...
        this.responseCheckPaymentStatus = await this.api_internetpayments_get(this.response.transactionId);
        if (this.responseCheckPaymentStatus.error) {
          ...
          winston.error("RunningTransaction[%d]::paymentChecks=%d, transactionID=%s", this.instance, paymentChecks, this.response.transactionId, code, description);

          var rtn = await this.opc.writeAsync(this.instance_config.errorCode, code);
          var rtn = await this.opc.writeAsync(this.instance_config.errorDescription, description);
          var rtn = await this.opc.writeAsync(this.instance_config.cancelBit, true);

          var rtn = await this.api_internetpayments_delete(this.response.transactionId);

          break;
        }

        // check if payment is canceled by PLC
        this.cancelBit = await this.opc.readAsync(this.instance_config.cancelBit);

        // -- if payment is cancelled, do api_internetpayments_delete
        if (this.cancelBit === true) {
          winston.warn("RunningTransaction[%d]::cancelBit=TRUE transactionId=%s", this.instance, this.response.transactionId, this.cancelBit);
          await this.api_internetpayments_delete(this.response.transactionId);
        }

        // if payment is signed or canceled break from the while loop
      ...
      if (this.responseCheckPaymentStatus.signed === true) {
        ...
        winston.info("RunningTransaction[%d]::requestCheckPaymentStatus-%d SIGNED %s", this.instance, paymentChecks, this.response.transactionId);
        
        // set 'transactionSigned' to TRUE
        var paymentToOPC = await this.opc.writeAsync(this.instance_config.transactionSigned, true);
        if (paymentToOPC === false) {
          winston.info("RunningTransaction[%d]::transactionSigned NOK %s", this.instance, this.response.transactionId);
          ...
          winston.info("RunningTransaction[%d]::transactionSigned OK %s", this.instance, this.response.transactionId);
        }
      }
  ...
  async api_internetpayments_post(paymentdata) {
    const me = this;
    return new Promise((resolve, reject) => {
      ...
      request({
        ...
        .then(function (data) {
          winston.debug('RunningTransaction[%d]::api_internetpayments_post OK', me.instance, url, JSON.stringify(data));
          resolve(data);
        })
        .catch(function (data) {
          winston.error('RunningTransaction[%d]::api_internetpayments_post NOK', me.instance, data.statusCode);
          resolve(data);
        });
    });
  }

  async api_internetpayments_get(transactionId) {
    const me = this;
    return new Promise((resolve, reject) => {
      ...
      request({
        ...
        .then(function (data) {
          winston.debug('RunningTransaction[%d]::api_internetpayments_get OK', me.instance, url, JSON.stringify(data));
          resolve(data);
        })
        .catch(function (error) {
          winston.error('RunningTransaction[%d]::api_internetpayments_get NOK', me.instance, error.statusCode);
          resolve(error);
        });
    });
  }

  async api_internetpayments_delete(transactionId) {
    const me = this;
    return new Promise((resolve, reject) => {
      ...
      request({
        ...
        .then(function (data) {
          winston.info('RunningTransaction[%d]::api_internetpayments_delete OK', me.instance, url, JSON.stringify(data));
          resolve(data);
        })
        .catch(function (error) {
          winston.error('RunningTransaction[%d]::api_internetpayments_delete NOK', me.instance, error.statusCode);
          resolve(error);
        });
    });
  }
